...
eid =strcmp(T.experiment,'Figure 4'); % filter for fig 4
T = T(eid,:);
...
% Plot the reported phosphenes in visual space (blue)
...
% Fit the electrode array to match phosphenes
p = optUW.fit('p2p_c.fitElectrodeGrid',p,{'ang','a','k','squish','xc','yc'},vx,vy);
p.shift = p.k*log(p.a);  % shouldn't really be a parameter

% Get the predictions
[err,predcx,predcy,cx,cy] = p2p_c.fitElectrodeGrid(p,vx,vy);

%% Draw the projected phosphenes (blue) and the best fitting array (green) in cortical space 
figure(2)
clf
hold on

% Plot cortical the grid
ecc = repmat(p.eccList,101,1);
ang = repmat(linspace(min(p.angList),max(p.angList),101)',1,length(p.eccList));
x = ecc.*cos(ang);
y = ecc.*sin(ang);
...
% Projected phosphenes (blue)
drawLocations(cx,cy,[0,0,1]);

% Best fitting array (green)
drawLocations(predcx,predcy,[0,1,0]);

%% go back and project best fitting electrode array back to visual space
[predvx,predvy] = p2p_c.c2v_real(p,predcx,predcy);

% overlay the projected best fitting array in visual space (green)
figure(1)
drawLocations(predvx,predvy,[0,1,0]);

%% Helper function for plotting points with labels
