...
standard_trl = p2p_c.define_trial(tp,standard_trl); % generate full pulse train
standard_trl= p2p_c.spike_model(tp, standard_trl); % simulate response
tp.thresh_resp = standard_trl.max_temporal_response;% use this as the threshold response
%return
...
for paperNum = 1:length(paper)
    fprintf('Paper: %s',paper(paperNum).name);
    ...
    for electrodeNum=1:length(paper(paperNum).electrode)
        standard_thresh = [];
        amp = [];
        fprintf('  Electrode: %s ',paper(paperNum).electrode{electrodeNum});
        for expNum=1:length(paper(paperNum).exp) % for each experiment in the paper
            fprintf('    Experiment: %5s',paper(paperNum).exp{expNum});
            
            % filter for data for current experiment and electrode
            eid = strcmp(T.experiment,paper(paperNum).exp{expNum}) & ...
                strcmp(T.electrode,paper(paperNum).electrode{electrodeNum});
            fprintf('        %d points',sum(eid));            
            Texp = T(eid,:);
            % estimate thresholds using model 
            [err, thresh] = p2p_c.loop_find_threshold(tp,Texp);         
            % concatenate the amplitudes at threshold for this electrode
            amp = [amp;Texp.amp];

            % simulate standard trial response for scaling
            Tsim = Texp;
...
% plot the model predicted standard thresholds
% pulse width graph
...
% plot experiment data points from papers
h.pd = [];
h.freq = [];
legStr.pd = {};
legStr.freq = {};
...
% for each paper
for paperNum = 1:length(paper)
    for expNum = 1:length(paper(paperNum).exp) % for each experiment
        
        if strcmp(paper(paperNum).exp{expNum}(1:2),'pd') % pulse duartion exp
            figure(1)
...
% pw vs. amplitude @ threshold
figure(1)
set(gca,'XTick',log(xtick.pd)); p2p_c.logx2raw; ylabel('Threshold');  grid; %widen;
xlabel('Pulse Width (msec)'); ylabel('Amplitude @ Threshold');
legend(h.pd,legStr.pd,'Location','NorthEast');
set(gca,'YLim',[0,12]);
set(gca,'FontSize',fontSize)

% compute correlation actual vs predicted
...
% freq vs. amplitude @ threshold
figure(2)
set(gca,'XTick',log(xtick.freq)); p2p_c.logx2raw(exp(1),0); ylabel('Threshold'); grid;%widen;
