...
nSamp = 1; % we are going to re-estimate the size of the percept multiple times. Error bars pretty low with 25
...
%% generate cortical surface, visual map and trial
...
for rad = 1:length(radList)
    ...
    for ecc = 1:length(eccList)
        ...
        v.e.ecc = eccList(ecc);
        v.e.ang = 0;
        for r = 1:nSamp % we are going to resample several times to estimate mean phosphene size
            [c, v] = p2p_c.generate_corticalmap(c, v); % create ocular dominance/orientation/rf size maps on cortical surface
            ...
            if r == 1 % plot an example phosphene
...
figure;
clist = parula(length(radList)+1);
hp = gobjects(length(radList),1); 

for rad = 1:length(radList)
    for ecc = 1:length(eccList)
        mn_sigma(rad, ecc) = mean(sigma(rad, ecc, :));
        std_sigma(rad,ecc) = std(sigma(rad, ecc, :))*2;
    end

    hp(rad) = plot(eccList, mn_sigma(rad, :), '.-',  ....
        'MarkerFaceColor', clist(rad,:), 'MarkerSize', 30, ...
        'LineStyle', '-', 'LineWidth', 2, 'MarkerEdgeColor', clist(rad,:), 'Color', clist(rad,:)); hold on
    sh = shadedErrorBar(eccList, mn_sigma(rad, :),  mn_sigma(rad, :));
...
xlabel('eccentricity');
ylabel('mean phosphene size: \sigma (degrees of visual space)');lg = legend(hp, cellstr(num2str(round(radList(:),2))));
