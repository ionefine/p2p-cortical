...
% keep rows with nonzero amp and defined brightness only
...
rng (11171964); % fix the random number generator
...
if FITFLAG
    ...
    tp = optUW.fit('p2p_c.fit_brightness',tp,freeParams,Texp);
end
...
% loop over electrode sites
for site =1:5
    eid =  find(T.electrode==site);
    if ~isempty(eid)
        Texp = T(eid, :);

        % run model for each trial
        trl= p2p_c.loop_model(tp, Texp);

        % force column vectors and handle NaNs
        x = [trl.max_temporal_response]; y = [Texp.brightness] ;
...
% overall correlation acroess sites
[r, p] = corr(allx, ally);
