...
% define color list
% roughly match colors to Winawer paper
colorList  = [ 0.5    0    0;   0.5 1 0.5;   1  0.8125    0; 0   0.8750   1;     0     0    1 ];
...
for ii=1:5
    ...
    % Global visual/cortical sampling and drawing threshold
    v.eccList = [1 2 3 5 8 13 21 34 80]; % eccentricity list 
    v. pixperdeg = 12;
    c.pixpermm = 12;
    v.drawthr = 1;

    % set up v and c for each case
    switch ii
        case 1
    ...
    % build maps and EF, then compute cortical response to the EF
    ...
    % pull all trials for this electrode with amp > 0 from the Winawer data table
    clear trl
    eid = intersect(find(T.electrode==ii), find(T.amp > 0) );
    Texp = T(eid,:);

    if ~isempty(Texp)
        trl = p2p_c.loop_model(tp, Texp); % do the time

        % generate phosphene model for each trial, plot, and collect metrics
        clear sim*
        for t = 1:length(trl)
            tmp_trl = trl(t);
            tmp_trl= p2p_c.generate_phosphene(v, tp, tmp_trl);
            p2p_c.plotretgrid(tmp_trl.max_phosphene(:, :, 1)*35, v, gray(256), 3,['subplot(2,1,1); title(''phosphene'')';]);
            ...
            maxphos(ct) = max(tmp_trl.max_phosphene(:)); ct = ct+1;
            drawnow
            ...
            if isfield(tmp_trl, 'ellipse')
                ...
                sim_brightness(t) = max(tmp_trl.max_phosphene(:));% max of both eyes
...
ylabel('Drawn Phosphene area (deg^2)');
p2p_c.logx2raw(10);  p2p_c.logy2raw(10);

% figure 2 titles, axis, etc.
figure(2)
for i = 1:2
    subplot(2, 1, i)
