...
classdef p2p_c
    methods(Static)
        ...
        function trl = define_trial(tp, varargin)
            ...
            if ~isfield(trl, 'ip');     trl.ip = trl.pw;             end % interphase delay
        ...
        function [trl_array,v] = generate_phosphene(v, tp, trl)
            ...
            % Ensure all fields that will be added exist in trl
            required_fields = {'spikeWhen', 'spikeStrength', 'max_phosphene', ...
                'sim_area', 'ellipse', 'sim_brightness', 'bin_max_phosphene'};
            ...
            for e = 1:n_electrodes
                ...
                if isnan(trl.freq) % if no frequency defined then just use a 1-scaled rf map
                    curr_trl.max_phosphene = v.e(curr_trl.e).rfmap;
                    curr_trl.spikeStrength = 1; % the scaling due to current integration
                else
                    % so we begin by calculating the convolution in time
                    % using the linear model, not the nonlinear model.
                    % That's because the nonlinear model has a response compression,
                    % but we only want to add this compression at the very
                    % end, AFTER we've calulated "spiking strength" for
                    % each point in space. 
                    % 
                    % If you don't put the compression at the very end phosphenes don't get
                    % larger with amplitude, because all phosphenes have
                    % the same max amplitude.The idea is that when
                    % amplitude goes up, a broader region of cortex reaches
                    % max amplitude. That requires the compression at the
                    % very end.
                    tmp = tp.saturation_model;
                    tp.saturation_model = 'linear';
                    curr_trl = p2p_c.spike_model(tp, curr_trl); % returns trl.resp (response over time) and trl.max_temporal_response
                    curr_trl.max_phosphene = v.e(curr_trl.e).rfmap.*curr_trl.max_temporal_response;
                    tp.saturation_model = tmp; % restore the nonlinearity
                    curr_trl.max_phosphene = p2p_c.nonlinearity(tp, curr_trl.max_phosphene);
                end

                % what rule to use to translate phosphene image to brightness?
               %  beta = 1; % soft-max rule across pixels for both eyes, if beta = 1 is the sum across the two eyes
                % curr_trl.sim_brightness = ((1/(v.pixperdeg.^2)) *
                % sum(curr_trl.max_phosphene(:).^beta)^(1/beta));  % IF
                % CHECK THIS V. DUBIOUS INDEED
                % % calculate the size of the image
                % curr_trl.sim_area = (1/v.pixperdeg.^2) * sum(curr_trl.max_phosphene(:) > v.drawthr)/2; 
                % % calculated area of phosphene based on max of left and right eyes
        ...
        function [tp, nc] = define_temporalparameters(varargin)
            ...
            if ~isfield(tp, 'adapt_on'); tp.adapt_on = 0; end
            if ~isfield(tp,'Na_recovery'); tp.Na_recovery = 0.015; end    % how quickly adaptation recovers
            if ~isfield(tp,'Na_strength'); tp.Na_strength = 500; end     % how strongly the adaptation attenuates Sodium (Na) currents
            ...
            if strcmp(tp.spikemodel, 'convolve')
                if ~isfield(tp, 'model');   tp.saturation_model = 'compression';   end
                if ~isfield(tp, 'sc_in');   tp.sc_in = 0.5663;   end
            elseif strcmp(tp.spikemodel, 'integratefire')
                if ~isfield(tp, 'model');   tp.saturation_model = 'linear';   end
                if ~isfield(tp, 'sc_in');   tp.sc_in = 1;  end % 10000/3;
            else
                error('model variant not defined, should be "compression" or "linear"');
            end

            if strcmp(tp.saturation_model, 'compression')
        ...
        function [loop_trl] = loop_model(tp,T, varargin)
            ...
            for i=1:size(T,1)
                ...
                if strcmp(tp.spikemodel, 'integratefire') && exist('nc')
                    loop_trl(i) = p2p_c.spike_model(tp, trl, nc); %  if using the spiking model and a defined number of cells
                else
                    loop_trl(i) = p2p_c.spike_model(tp, trl); % calling either convolve model or spiking model without a defined number of cells
                end
            end
        end
        ...
        function [trl, nc,v] = integratefire_model(tp, trl, nc)
            ...
            nt = length(trl.t);
            dt = tp.dt;
            Iinput = trl.pt * 1000; % *1000000;  % picoamps to microamps
            % if tp.adapt_on
            % trl = p2p_c.Na_adapt(tp, trl);
            % else
            %   trl.adapt = ones(size(trl.pt));
            % end
            % zero out vector to store membrane potential (v) and adapation variable (w)

            names = fieldnames(tp);
            for i=1:length(names)
                eval(sprintf('%s = tp.%s;', names{i},names{i}))
            end
            trl.indexFire = NaN(length(trl.pt), length(nc));
            for cc = 1:length(nc)
                clear names
                ...
                for timei = 1:(nt-1)  % simulation in loop over time
                    ...
                    v(timei+1) = v(timei) + dt*(-gl*(v(timei)-El) + ...
                        gl*delT*exp((v(timei)-vt)/delT) + ...    %   ( (1./(1+(Na_strength.*Na_adapt(timei)))) * ...
                           (Na_adapt(timei)*Iinput(timei)) - w(timei)) * R;
                    ...
                    tmp(timei+1) =   tmp(timei) + dt*max(Iinput(timei), 0) - ((dt/Na_recovery).*tmp(timei));
                    Na_adapt(timei+1) = 1/(1+Na_strength*tmp(timei+1));
                ...
                trl.indexFire(:, cc) = v == nc(cc).vpeak; % a matrix showing when each cell fired
            end
            if length(nc)>1
                trl.spikeWhen = find(sum(trl.indexFire,2));
                trl.spikeStrength =  sum(trl.indexFire(trl.spikeWhen,:),2)./length(nc); % this not really helpful since spikes are distributed in time
            else
                trl.spikeWhen = find(trl.indexFire);
                trl.spikeStrength = trl.indexFire(trl.spikeWhen);
            end
            trl.Na_adapt = Na_adapt; % faster doing this after the loop
         
        end
        function trl = Na_adapt(tp, trl)
            ...
            h = p2p_c.gamma(tp.ncascades,tp.Na_recovery,trl.t);            % Generate the n-cascade impulse response
            ...
            trl.adapt = tp.Na_strength * conv(max(trl.pt, 0),h,  'same');
        end

       % function trl = Na_adaptation(tp, trl, nc)
        ...
        function nc = define_integratefirecells(varargin)
            ...
            if nargin == 1 & isstruct(varargin{1})
                ...
                for cc = 1:nCells
                    ...
                    if ~isfield(nc, 'R')  || isempty(nc(cc).R);      nc(cc).R = .25 - log(rand(1))/2; end 
                    % resistance of cells as they get further from electrode, assume cells have a flat distribution as a function of distance from electrode, and that current falls off exponentially
                end
            else
                if nargin==0
                    nCells = 1;
                ...
                for cc = 1:nCells  % populate any missing values
                    ...
                    nc(cc).tARP = 0;% 5*10^-3; % 5*10^-3;  % absolute refractory period
                    nc(cc).R = .25 - log(rand(1))/2; 
                end
            end
        end
        ...
        function [trl, nc] = spike_model(tp, trl, varargin)
            ...
            %% run the convolve or the AdEx Spiking model
            if strcmp(tp.spikemodel, 'convolve')
                trl = p2p_c.convolve_model(tp, trl);
            elseif strcmp(tp.spikemodel, 'integratefire')
                if nargin>2
            ...
            % at this point you should have trl.spikeStrength, a discrete
            % set of events with a "spiking strength" at those discrete
            % points in time

            if tp.gammaflag
                trl = p2p_c.slowgamma(tp, trl); % pass through slow gamma to get a slow response, trl.resp
            else
                trl.resp = zeros(size(trl.pt)); % if no slow gamma put the "spikes" into trl.resp
                trl.resp(trl.spikeWhen) = trl.spikeStrength;
            end

            trl.resp = p2p_c.nonlinearity(tp,trl.resp);
            trl.max_temporal_response = max(trl.resp);
            trl.resp = trl.resp(1:length(trl.pt));
            if  max(trl.resp)>trl.max_temporal_response
                error('trial has been cut off before peak response. Increase trl.simdur');
            end           
        end
        ...
        function trl = convolve_model(tp, trl)
            ...
            % Since pulses are sparse, manually convolve the 'pulses' with
            ...
            trl.spikeWhen = find(diff(trl.pt))+1;

            % R will hold the values of R1 at the event times.
            Rtmp = zeros(1,length(trl.spikeWhen));
            wasRising = 0;
            ...
            for i=1:(length(trl.spikeWhen)-1)
                %    tNow = trl.t(spikeWhen(i+1));
                delta = trl.t(trl.spikeWhen(i+1))-trl.t(trl.spikeWhen(i));  % time since last 'event'
                % Closed form solution to leaky integrator that predicts
                % R(i+1) from R(i), delta and tau1:
                Rtmp(i+1) = trl.pt(trl.spikeWhen(i))*tp.tau1*(1-exp(-delta/tp.tau1)) + ...
                ...
                if Rtmp(i+1)<Rtmp(i) && wasRising
                    spikeId(i) = 1; % check spikeId identical in both loops
            ...
            R1 = Rtmp * 1000; % is this multiplied by 1000 I don't know
            R1(R1<0)=0;
            if isempty(R1) % no pulses at all
                ...
                spikeId(R1<=0) = 0; % sometimes non-spiking events identified as spiking events
                % pull out only spike events
                trl.spikeStrength = R1(spikeId);
                trl.spikeWhen = trl.spikeWhen(spikeId); % times of "spiking events"
            end
        end
        ...
        function trl = slowgamma(tp, trl)
            ...
            tid = find(cumsum(h)*tp.dt>=.998,1,'first');      % Shorten the filter if needed to speed up the code.
            if ~isempty(tid)
                h = h(1:tid);
                trl.imp_resp = h;  % close enough to use h
            else
            ...
            if  strcmp(tp.spikemodel, 'convolve')
                ...
                % integrate and fire model
                interspike = [1,diff(trl.t(trl.spikeWhen))];
            ...
            for i=1:length(trl.spikeStrength)
                id = find(trl.t>trl.t(trl.spikeWhen(i)),1,'first');
                resp(id+impFrames)  =   ...
