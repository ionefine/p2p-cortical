...
classdef p2p_c
    methods(Static)
        ...
        function v = define_visualmap(v)
            % defines the height, width, resultion etc of the visual field that is being simulated
            %
            % commented 12/7/2024 IF
        ...
        function [v, c] = generate_corticalelectricalresponse(c, v)
            ...
            for ii = 1:length(idx) % for each electrode
                disp(['Generating cortical electrical response. This may take a while ...'])
        ...
        function [trl,v] = generate_phosphene(v, tp, trl)
            ...
            if isnan(trl.freq)
                trl.max_phosphene = v.e(trl.e).rfmap; trl.resp = 1; % the scaling due to current integration
            else
                trl = p2p_c.convolve_model(tp, trl);
                trl.max_phosphene = v.e(trl.e).rfmap.*max(trl.resp);
            end
        ...
        function [err, thresh] = loop_find_threshold(tp,trl, varargin)
            if nargin ==3
                nc = varargin{1};
            end
            %
        ...
        function amp = find_threshold_lif(trl, tp, nc)
            ...
            for i = 1:tp.nReps
                trl.amp  = (hi+lo)/2;
                trl = p2p_c.define_trial(tp,trl);
                nc = p2p_c.spike_model(nc, trl, tp);

                if ~isnan(nc.indexFire).*(sum(nc.indexFire(:))>tp.thresh_resp)
                    hi = trl.amp;
                else
                    lo = trl.amp;
                end
        ...
        function nc = integratefire_model(tp, trl, varargin)
            % adapted from
            % https://d-arora.github.io/Doing-Physics-With-Matlab/mpDocs/pbs007.htm
            % Ian Cooper
            if nargin ==2
                nc.nCells = 1;
            else
            ...
            if ~isfield(nc, 'nCells'); nc.nCells = 1; end
            if ~isfield(nc, 'R'); (10+R_std*randn(1,nCells))*10^6;  end   % membrane resistance  [10e6 ohms]
            if ~isfield(nc, 'R_std'); nc.R_std = 0; end           % variation in the resistance of cells
            if ~isfield(nc, 'vTH'); nc.vTH = -50;   end         % threshold voltage,
            if ~isfield(nc, 'vReset'); nc.vReset = -80;  end      % reset potential
            if ~isfield(nc, 'vRest'); nc.vRest = -60;  end  % rest potential
            if ~isfield(nc, 'vSpike'); nc.vSpike = 20; end   % spike potential
            if ~isfield(nc, 'Ifac'); nc.Ifac = 1.2*10^-6;  end % multiply electrode current by this to get Iext, tp.amp x nc.Ifac = Iext

            if ~isfield(tp, 'tARP'); nc.tARP = 4*10^-3; end % absolute refractory period
            if ~isfield(tp, 'thresh_resp'), tp.thresh_resp = ceil(nCells/2); end % if 1 cell, it must fire to reach threshold, otherwise the default is half the cells must fire


            nc.V = repmat(nc.vRest .* ones(1,length(trl.pt)), length(nc.R), 1); % vM  initialize membrane voltage  [mV]
            K = tp.dt./tp.tau1;
            nARP = round(tp.tARP / tp.dt);
            nc.Iext = trl.pt.*nc.Ifac;
            nc.Vext = nc.R' * nc.Iext; % creates a n x ntps matrix, for cells of varying voltages
            d = -1*ones(size(nc.R));
            for c = 1:length(trl.pt) - 1
                nc.V(:, c+1) = nc.V(:, c) +  K .* (-nc.V(:, c) + nc.vRest + nc.Vext(:, c));
                ind = find(nc.V(:, c+1) > nc.vTH);
                nc.V(ind,c) = nc.vSpike;
                nc.V(ind,c+1) = nc.vReset;
                d(ind) = nARP;

                ind2 = find(d>0);
                nc.V(ind2, c+1) = nc.vReset;
                d(ind2)=d(ind2)-1;
            end
            nc.indexFire = nc.V == nc.vSpike;
        end
        function nc = suppressfiring(tp, trl, nc)

            if ~isfield(nc, 'tauS'); nc.tauS = 0.25; end
            if ~isfield(nc, 'strengthS'); nc.strengthS = 1; end
            nc.suppression = repmat(zeros(1,length(trl.pt)), length(nc.R), 1);
            % models the suppression of restoration of resting state firing as
            % a result of electrical stimulation

            rVext= nc.Vext.*tp.dt; rVext(rVext<0) = 0;
            for c = 1:length(trl.pt) - 1
                nc.suppression(:, c+1) =  (nc.suppression(:, c) + rVext(:, c)) - ((tp.dt/nc.tauS).*nc.suppression(:, c)); % assume suppression driven by charge
            end
            nc.suppression = nc.suppression.*nc.supStrength;
        end
        ...
        function amp = find_threshold(trl, tp, varargin)
            ...
            while resp<tp.thresh_resp
                ...
                trl.amp = hi;
                trl = p2p_c.define_trial(tp,trl);
                if strcmp(tp.model, 'integratefire')
                    if exist('nc')
                        trl = integratefire_model(tp, trl, nc);
                    else
                        trl = integratefire_model(tp, trl);
                    end
                elseif strcmp(model, 'convolve')
                    trl = p2p_c.convolve_model(tp, trl);
                end
                resp = max(trl.resp);
            end
        ...
        function trl = spike_model(tp, trl, varargin)
            % Two variations for predicting neural / perceptual responses to
            ...
            if strcmp(tp.model, 'convolve')
                trl = convolve_model(tp, trl);
            elseif strcmp(tp.model, 'integratefire')
                if exist('nc')
                    trl = integratefire_model(tp, trl, nc);
                else
                    trl = integratefire_model(tp, trl);
        ...
        function trl = slowgamma(tp, trl)
            ...
            tid = find(cumsum(h)*tp.dt>=.999,1,'first'); % Shorten the filter if needed to speed up the code. IF CHANGE
            if ~isempty(tid)
                h = h(1:tid);
            else
